...
This document describes the technical architecture for the Zomato-inspired restaurant recommendation service. It is derived from [context.md](./context.md) and defines components, data flows, interfaces, and implementation guidance for a greenfield build.

**Related:** [IMPLEMENTATION_PLAN.md](./IMPLEMENTATION_PLAN.md) (phased build) · [EDGE_CASES.md](./EDGE_CASES.md) (edge-case behaviors and test IDs)
...
**Milestone LLM provider:** [Groq](https://console.groq.com/) (OpenAI-compatible Chat Completions API). OpenAI, Anthropic, and Ollama remain optional alternates via `LLMClient` abstraction.
...
Thin wrapper over **Groq** (milestone default) or swappable providers (OpenAI, Anthropic, Ollama)
...
**Milestone default** — fast inference via Groq Chat Completions (`https://api.groq.com/openai/v1`)

OpenAIClient

Optional alternate (same client pattern as Groq)
...
Optional local dev / offline demo

Factory

Select implementation from `LLM_PROVIDER` env (`groq` default)

Groq (Milestone Provider)
...
Groq OpenAI-compatible REST (`/chat/completions`)
...
`llama-3.3-70b-versatile` (override via `LLM_MODEL`)

SDK / HTTP

`groq` Python SDK or `openai` client with `base_url=https://api.groq.com/openai/v1`

Structured output

Use JSON mode / `response_format` when model supports it; parser validates regardless

Typical latency

Often under 2 seconds for capped candidate prompts (still show loading indicator)
...
`groq` (milestone default) · `openai` · `anthropic` · `ollama`

LLM_API_KEY

Provider secret (Groq API key when `LLM_PROVIDER=groq`)
...
Optional override; default `https://api.groq.com` (Groq SDK adds `/openai/v1` paths)
...
**Groq API** (milestone); Ollama optional for offline dev
